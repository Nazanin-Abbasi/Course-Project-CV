...
# # Example usage
# input_image_path = "ukulele2_frames/frame_71.png"  # Path to your input PNG image
# output_image_path = (
#     "cropped_ukulele/cropped_frame_71.png"  # Path to save the cropped image
# )
...
def draw_polygon(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEMOVE and len(polygon_points) > 0:
        # Optionally, visualize the current drawing during the mouse movement
    ...
    if event == cv2.EVENT_RBUTTONDOWN and len(polygon_points) > 2:
        # If the right mouse button is clicked, close the polygon
        ...
        cv2.imshow("Draw Polygon", param["image"])
        print("Polygon points: ", polygon_points)
...
def crop_polygon_region(input_image_path, output_image_path):
    ...
    # Set the mouse callback function to capture the points
    cv2.imshow("Draw Polygon", temp_image)
    cv2.setMouseCallback("Draw Polygon", draw_polygon, param={"image": temp_image})

    # Wait for the user to draw the polygon
    print("Draw the polygon. Right-click to close.")
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    ...
    if len(polygon_points) > 2:
        ...
        print("Insufficient points to create a polygon. Please try again.")


# Example usage
input_image_path = "ukulele2_frames/frame_184.png"  # Path to your input PNG image
output_image_path = (
    "best_ukulele/cropped_frame_184.png"  # Path to save the cropped image
)
crop_polygon_region(
